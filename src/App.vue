<template>
  <div class="bg-gray-900 text-white min-h-screen flex flex-col items-center justify-center font-sans p-4">
    ...
    <div v-if="!playerName" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-gray-800 p-8 rounded-xl shadow-2xl max-w-md w-full mx-4">
        <h2 class="text-2xl font-bold text-emerald-400 mb-4 text-center">Bem-vindo ao Campo Minado!</h2>
        <p class="text-gray-300 mb-6 text-center">Digite seu nome para começar a jogar:</p>
        <input
    ...
    <!-- Ranking -->
    <div v-if="playerName && rankings.length > 0" class="mt-8 bg-gray-800 p-6 rounded-xl shadow-lg max-w-2xl w-full">
      <h3 class="text-2xl font-bold text-emerald-400 mb-4 text-center">🏆 Ranking</h3>
      <div class="space-y-2">
        <div 
        ...
        >
          <div class="flex items-center space-x-3">
            <span class="text-lg font-bold text-emerald-400">{{ index + 1 }}º</span>
            <span class="font-medium">{{ rank.playerName }}</span>
          </div>
          <div class="text-right">
            <div class="text-sm text-gray-300">{{ rank.boardSize }} células</div>
    ...
    <div v-if="playerName" class="mt-8 text-center text-gray-400 max-w-xl">
      ...
      <p class="mt-2 text-sm">O tabuleiro se expande automaticamente quando você se aproxima das bordas!</p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    saveRankings() {
      localStorage.setItem('minesweeper_rankings', JSON.stringify(this.rankings))
    },
